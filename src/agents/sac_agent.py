...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = nn.Parameter(
                ptu.from_numpy(np.array([np.log(self.temperature)], dtype=np.float32))
            )
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            #The actions are now continous, so we can't pick action based on the Q function, so we have a policy and let her learn the best action!
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                #Entropy is used as "exploration" in DQN we had e with is a greedy param for
                # exploration but here we have continous actions, and use entropy as "exploration"
                # metric
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        loss = (self.get_temperature() * log_prob.detach() - q_values.min(dim=0)[0]).mean()
        # ENDTODO
    ...
    def update(
        ...
        if self.target_update_period is not None:
            #Here we have hard update ( every N steps)
            if step % self.target_update_period == 0:
                self.update_target_critic()
        else:
            #Here we have soft update
